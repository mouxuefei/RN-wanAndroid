...
export class AdvancePage extends Component {
    ...
    loadData(isLoadmore) {
        const url = "https://wanandroid.com/article/list/"+ this.state.pageNum +"/json?cid=375";
                ...
                if (isLoadmore) {
                    if (data.length === 0 || data.length < 15) {
                        this.setState({
                            ...
                            advanceData: [{
                                key: "advanceData",
                                data: [...this.state.advanceData[0].data, ...data],
                            },],
                        });
                    } else {
                        this.setState({
    ...
    render() {
        ...
        return <View style={{flex: 1}}>
            <TitleBar title='趋势' right='' navigation={navigation} />
            {this.getSectionList()}
        </View>
    }

    getSectionList() {
        if (this.state.advanceData[0].data == null) {
            return <Loading/>
        } else if (this.state.advanceData[0].data === 0) {
            return <Empty/>
        } else {
            return <SectionList
                ...
                sections={this.state.advanceData}
